...
import { fetchUserDetails } from "../../app/slice/userSlice";
import { FaSpinner, FaEdit, FaTrash } from "react-icons/fa"; // ✅ הוספה
import { appColors } from "../../utils/colors";
import EditAdModal from "./EditAdModal";
import { useWindowSize } from "../../utils/hooks";
import { sendMail } from "../../utils/mailService";
...
export default function AdsManager() {
  ...
  const [filter, setFilter] = useState<"all" | "notApproved" | "notRelevant">(
    "all"
  );
  const [togglingId, setTogglingId] = useState<number | null>(null);
  const [editingAd, setEditingAd] = useState<any | null>(null);
  ...
  const handleToggleRelevant = async (ad: any) => {
    ...
    try {
      ...
      if (userEmail) {
        ...
        const subject = isNowRelevant
          ? "המודעה שלך נחשבה רלוונטית"
          : "המודעה שלך נחשבה לא רלוונטית";
        const text = isNowRelevant
          ? "המודעה שלך נחשבת כעת רלוונטית במערכת."
          : "המודעה שלך נחשבת כעת לא רלוונטית במערכת.";
        await sendMail({ to: userEmail, subject, text });
  ...
  const styles: Record<string, React.CSSProperties> = {
    // ... (כל הסגנונות שלך נשארו בדיוק אותו דבר)
    // לא שיניתי כלום חוץ מהוספת כפתור פח בהמשך.
  };
  ...
  const getToggleStyle = (isActive: boolean, isDisabled: boolean) => {
    const containerStyle: React.CSSProperties = {
      width: "44px",
      height: "22px",
      borderRadius: "11px",
  ...
  return (
    <div style={{ padding: "20px", direction: "rtl" }}>
      <AnimationStyles />
      <h1 style={{ textAlign: "center", color: colors.primary }}>ניהול מודעות</h1>
      ...
      <div style={{ marginTop: "30px", display: "grid", gap: "20px" }}>
        {ads.map((ad) => (
          ...
          >
            <h3 style={{ color: colors.primaryDark }}>{ad.company}</h3>
            <p>{ad.description}</p>
